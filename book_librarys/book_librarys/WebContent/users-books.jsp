...
<html lang="en">
	...
	<body>
						...
						<tbody>
							...
							<tr>
								...
								<td class="td-status">
	                                ...
	                                <td>${book.press }</td>
	                                <td>${book.num }</td>
	                                 <td>${book.num - book.residuals }</td>
								<td class="td-manage">
									<c:choose>
									<c:when test="${(book.num - book.residuals) gt 0 }">
									<a title="预定" href="javascript:;" onclick="member_stop('${book.bname }','${book.bid }','${sid }')" class="ml-5" style="text-decoration:none">
										<span class="layui-btn layui-btn-normal layui-btn-mini">点击预定</span>
									</a>
									</c:when>
									<c:otherwise>
									暂无余书
		...
		function exit() {
			...
			/*用户-停用*/
			function member_stop(bname,bid,sid) {
					$.ajax({
			    		url:"Student",
			    		type:"post",
			    		data:{
			    		...
			    		success:function(result){
			    			...
			    			if(result == "1"){
			    				var flag="确认预定《"+bname+"》?";
			    				layer.confirm(flag, function(index) {
				    				window.location.href ="History?method=history_add&bid="+bid+"&sid="+sid;
				    				//发异步把用户状态进行更改
									layer.msg('成功!', {
										icon: 1,
										time: 1000
			    				...
			    				layer.msg('你的信用分不够！请联系管理员！', {
									icon: 5,
									time: 1000
								});
			    			}else if(result == "3"){
			    				layer.msg('你已经借超过上限！请先还书！', {
									icon: 5,
									time: 1000
								});
			    			}else if(result == "4"){
